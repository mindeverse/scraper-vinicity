...
class SupabaseClient:
    ...
    def _clean_row(row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            if v is not None and k != "embedding_version"
        }
        ...
        for i, batch in enumerate(batches):
            ...
            for attempt in range(3):
                try:
                    self.client.table("products").upsert(
                        [self._clean_row(r) for r in batch], on_conflict="source,product_url"
                    ).execute()
                    ok += len(batch)
                    batch_ok = True
                    break
            ...
            for row in batch:
                ...
                for attempt in range(3):
                    try:
                        self.client.table("products").upsert(
                            self._clean_row(row), on_conflict="source,product_url"
                        ).execute()
                        ok += 1
                        row_ok = True
                        break
