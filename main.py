"""VICINITY scraper — scrape Shopify products.json, embed with local SigLIP, upsert Supabase."""
...
logger = logging.Logger("scraper-vinicity")
...
def _to_db_row(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if record.get("back_image_embedding"):
...
def run() -> dict[str, Any]:
    logger.info("=== VICINITY scraper start ===")
    ...
    for k, v in summary.items():
        logger.info("%s: %s", k, v)
    logger.info("=== VICINITY scraper complete ===")
    return summary
